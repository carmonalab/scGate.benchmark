...
ddir <- "~/scGate.benchmark/aux/"
dir.create(ddir, showWarnings = FALSE)
options(timeout=1000)
if(TRUE){
  #subset 50K Hao et.al. 2021
  rds.file <- "~/aux/Hao_subset50K_sct.rds"
  if (!file.exists(rds.file)) {
    dataUrl.rds <- "https://figshare.com/ndownloader/files/31347367?private_link=31c57715c30ce9f57552"
  ...
  saveRDS(pbmc.azimuth.subset,file = "~/aux/Hao_subset50K_sct.rds") # This file is which is now in figshare
}

#Reduce object size
pbmc.azimuth.subset@assays$RNA <- pbmc.azimuth.full@assays$SCT
...
saveRDS(results,"~/aux/runtime_SingleR_scGate_4core.rds")
...
save = FALSE
if(save){
  ggsave(filename = "~/aux/runtime_benchmark_onDocker_4c.png",plot = plt, width = 7,height = 7)
  ggsave(filename = "~/aux/runtime_benchmark_onDocker_4c.pdf",plot = plt, width = 7,height = 7)
}
```
