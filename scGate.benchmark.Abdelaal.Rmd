...
```{r}
dset <- "/Users/mass/Dropbox/CSI/Datasets/benchmarking/Abdelaal2019/Inter-dataset/PbmcBench/seurat/pbmcs_abdelaal2019.rds"
testing.datasets <- readRDS(dset)
platforms = c("10Xv2","10Xv3","CEL-Seq","Drop-Seq","inDrop","Seq-Well","Smart-Seq2")

keep_pbmc1 <- grep("^pbmc1",names(testing.datasets))
testing.datasets <- testing.datasets[keep_pbmc1]
gc()
...
library(patchwork)
pll <- list()
for (i in seq_along(testing.datasets)) {
   name <- names(testing.datasets)[i]
   set <- testing.datasets[[i]]
   
   pll[[name]] <- DimPlot(set, group.by = "Label") + ggtitle(name)
...
```{r}
saveRDS(results,"./aux/results.abdelaal.rds")
```

```{r}
results.to.plot <- results%>%subset(method!="Garnett.clust.ext" & metric!= "REC")
...
ordered.methods <- c("scGate","SingleR","SCINA","Garnett")
...
#ggsave(file.path(figure.path,"benchmark_20012022_scGate_portr.pdf"), plot=b, width=6, height=4)
ggsave(file.path(figure.path,"benchmark_abdelaal_technology_scGate_portr.pdf"), plot=b, width=6, height=4)
